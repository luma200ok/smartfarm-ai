...
def test_daily_coach_returns_schema(monkeypatch):
    monkeypatch.setattr(pipeline, "get_forecast",
                        lambda window=None: {"next_temp": 30.0, "trend": "유지", "humidity_risk": "보통", "humidity_mean": 70.0})
...
def test_early_warning_high_humidity_uses_leafmold_rag(monkeypatch):
    monkeypatch.setattr(pipeline, "get_forecast",
                        lambda window=None: {"next_temp": 30.0, "trend": "유지", "humidity_risk": "높음", "humidity_mean": 92.0})
    seen = {"disease": None}
...
def test_early_warning_low_humidity_skips_rag(monkeypatch):
    monkeypatch.setattr(pipeline, "get_forecast",
                        lambda window=None: {"next_temp": 24.0, "trend": "유지", "humidity_risk": "낮음", "humidity_mean": 45.0})
    calls = {"n": 0}
...
def test_early_warning_no_forecast_returns_normal(monkeypatch):
    monkeypatch.setattr(pipeline, "get_forecast", lambda window=None: {"unavailable": True, "reason": "x"})
    w = pipeline.early_warning()
