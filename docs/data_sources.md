...
| **내용** | 시설채소 농가의 **환경·생육·생산·재배정보** (9작물: 완숙/방울토마토·딸기·오이·참외·파프리카·가지·국화·수박, 2020~2024) |
| **사용 범위** | **2022~2024 환경**(288만 시간별) → **일별 집계 116,365행** → 작물 9종 분류 (수박은 2023~24만) |
...
| **확장 후보** | ✅ 다년치(2022~2024) 결합 완료 → 2020~2021 추가 여지 · AI Hub 지능형 스마트팜 통합데이터(토마토 534) 환경 시계열 |
...
| **비전(병진단) ② ★실사용** | **AI Hub 「시설작물 질병진단」(071)** — https://aihub.or.kr/aihubdata/data/view.do?dataSetSn=153 (토마토 정상·잎곰팡이병(18)·황화잎말이(19) + **부위 라벨 `area`**). 정상 원천의 부위 혼재 → **잎(area=3)만 정제**해 3분류 학습(train 3,255·val 691) |
...
> 전략: PlantVillage로 파이프라인 완성 → **AI Hub 071**(국내)로 교체·고도화 (전이학습). 데이터 맵 → [decisions.md](decisions.md). · ※ 계획 문서의 "153"은 실제 사용한 **071 「시설작물 질병진단」** 으로 정정.
...
- **AI Hub 071(Phase2)**: 한국 국적 회원 신청 → 승인 후 다운 (Training/Validation + 원천, **토마토만** 선택). 폴더 `071.시설 작물 질병 진단/`
```
